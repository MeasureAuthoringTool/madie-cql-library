  ...
  const saveReview = async (values: {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("review-cql-library-saved", { detail: savedReview })
      );
      onClose();
    } catch (error) {
      setToast({
    ...
    <>
      ...
      >
        ...
        <div data-testid="review-dialog-remove-confirmation-message">
          You are about to remove this library from the review process which is
          {" already "}
          {
            STATUS_DISPLAY_TEXT[
          ...
          . Any existing comments on the library will be retained.
        </div>
        <Divider sx={{ mt: 2 }} />
      </MadieDialog>
      <Toast
